...
<!-- gitnexus:start -->
# GitNexus — Code Intelligence (secondary / optional)

> **Tool precedence.** CodeGraph is this project's **primary** code-intelligence
> tool — see the CodeGraph section in `.claude/CLAUDE.md`. Reach for it first for
> "how does X work", locating code, and impact / blast-radius questions.
> GitNexus (below) is a **secondary, optional** tool kept for graph features
> CodeGraph doesn't have — execution-flow *processes*, Cypher queries, and
> `detect_changes`. Nothing below is a required gate on editing; use GitNexus
> when its graph view adds something, not on every change.
>
> This block sits between GitNexus's own auto-managed markers. Re-running
> `gitnexus install` can regenerate it with the original mandatory ("MUST run
> impact before every edit") wording — if that happens, re-apply this
> secondary/optional framing.

This project is indexed by GitNexus as **patas-arriba-monorepo**. The index is
local and gitignored (`.gitnexus/`) and lags the working tree; read live stats
from `.gitnexus/meta.json` rather than a hardcoded count, and if a tool warns
the index is stale, run `npx gitnexus analyze` first.

## When GitNexus adds something CodeGraph doesn't

- **Execution flows / processes** — `gitnexus_query({query: "concept"})` returns
  process-grouped results; `READ gitnexus://repo/patas-arriba-monorepo/process/{name}`
  traces a flow step by step.
- **Custom graph queries** — `gitnexus_cypher({query: "MATCH ..."})`.
- **Branch scope check** — `gitnexus_detect_changes({scope: "compare", base_ref: "main"})`
  to see what a branch changed at the symbol / flow level.
- **Guided rename** — `gitnexus_rename({symbol_name, new_name, dry_run: true})`.
...
| `impact` | Blast radius before a risky edit | `gitnexus_impact({target: "X", direction: "upstream"})` |
| `detect_changes` | Scope check | `gitnexus_detect_changes({scope: "staged"})` |
| `rename` | Safe multi-file rename | `gitnexus_rename({symbol_name: "old", new_name: "new", dry_run: true})` |
| `cypher` | Custom graph queries | `gitnexus_cypher({query: "MATCH ..."})` |

## Keeping the Index Fresh

GitNexus's index is separate from CodeGraph's. Refresh it only if you're
actively using GitNexus in a session:
